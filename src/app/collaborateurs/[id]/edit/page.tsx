...
export default function EditCollaboratorPage() {
  ...
  useEffect(() => {
    if (params.id) {
      supabase.from('collaborators').select('*').eq('id', params.id).maybeSingle()
        .then(({ data }) => {
          if (!data) return;
          const row = data as { name?: string; phone?: string | null; email?: string | null; address?: string | null; notes?: string | null; role_position?: string | null };
          setFormData({
            name: row.name || '',
            phone: row.phone || '',
            email: row.email || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { error } = await (supabase.from('collaborators') as any).update({
  ...
  return (
    <AppLayout>
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <h1 className="text-2xl font-bold text-text">Modifier le collaborateur</h1>
        <Card>
